...
| Version | Key Updates |
|---------|-------------|
| **v3.2.8** | **Motion Preference and Control State Patch**: Respected `prefers-reduced-motion` across global CSS motion, scroll reveal, counter animation, and home hero parallax; synchronized `aria-pressed` for album, links, tag, and archive segmented controls with quality gates; reused the shared scripts partial on the login page to avoid duplicated CDN/SRI script maintenance. |
| **v3.2.7** | **Interaction State and Modal Stability Patch**: Hid decorative archive heatmap cells and legend markers from assistive technology; synchronized `aria-pressed` for the post grid/list view toggle and added a quality gate; added roving `tabindex` to the login/register tabs; hardened the account deletion modal close path and fallback behavior; expanded Playwright smoke coverage. |
