...
| 版本 | 主要内容 |
|------|---------|
| **v3.2.8** | **动态偏好与控件状态补丁版**：尊重 `prefers-reduced-motion`，为全局 CSS 动效、滚动 reveal、计数动画与首页视差提供减少动态效果降级；为相册、友链、标签、归档年份等分段控件同步 `aria-pressed` 状态并增加质量门禁；登录页改为复用共享 scripts partial，避免 CDN/SRI 脚本引用重复维护。 |
| **v3.2.7** | **交互状态与弹窗稳定性补丁版**：隐藏归档热力图与图例中的装饰单元，避免辅助技术朗读大量无意义格子；为文章列表网格/列表视图切换补齐 `aria-pressed` 状态同步与质量门禁；为登录/注册 Tab 增加 roving `tabindex`；强化账号注销 modal 的关闭路径与 fallback 兜底，并扩展 Playwright smoke 覆盖。 |
